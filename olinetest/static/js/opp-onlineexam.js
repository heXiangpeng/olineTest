...
online.score_datatable = function (userid) {
    $("#score_datatable").dataTable({
        "bDestory": true,
        "sAjaxSource": global_RootPath+"/scores/userid?userid=" + userid,
        "sAjaxDataProp": "data",
        "aoColumns": [
            {"sDefaultContent": ''},
            {"mDataProp": "examname"},
...
online._btn_start_exam = function (examid) {
    online.index =  layer.open({
        ...
        content: '/open?examid='+examid, //iframe的url
        end:function(){
            var cookieUse=guodandan.userCookie('sessionUser');
            online.clearTable_score_datatable(cookieUse.userid);
...
online._btn_history_exam = function (examid) {
    ...
    $.ajax({
        type: "GET",
        url: global_RootPath+"/records/history?examid=" + examid,
        dataType: "json",
        success: function (datarc) {
            if (datarc.rc == 0) {
                var redata=datarc.data;
                ...
                var multiple=redata.multiple;
                var judge=redata.judge;
                var tiangkong=redata.tiangkong;
                online.RenderhistoryExam(single,multiple,judge,tiangkong);
            } else {
                guodandan.notAuth(data);
            }
        }
...
online.RenderExam=function(single,multiple,judge,tiangkon){
    ...
    if(judge!=null&&judge!=""){
        ...
        $("#judge_examlib").append(judgestr);
               ...
               //填空
        if(tiangkon!=null&&tiangkon!=""){
            $("#tiankong_total").val(judge.length);
            var judgestr=""
            $("#tiankong_type").html("填空题（共"+tiangkon.length+"题，1题"+tiangkon[0].weight+"分)");
            for(var i=0;i<judge.length;i++){
                judgestr+='<strong> <h4>'+tiangkon[i].sort+'、<span>'+tiangkon[i].title+'</span></h4><h5> <input type="text" name="'+tiangkon[i].id+'" value="1"> </h5> </strong>';
            }
            $("#tiankong_examlib").append(judgestr);
        }

}
online.RenderhistoryExam=function(single,multiple,judge,tiangkong){
    $("#single_examlib").empty();
    $("#multiple_total").empty();
    $("#judge_total").empty();
    //单选题
    if(single!=null&&single!=""){
        ...
        for(var i=0;i<single.length;i++){
            var options = single[i].options.split("##")
            singlestr+='<strong> <h4>'+single[i].sort+'、<span>'+single[i].title+'</span></h4><h5> <span> <label class="radio"> <input type="radio" name="'+single[i].id+'" value="A" >'+options[1]+'</label><label class="radio"> <input type="radio" name="'+single[i].id+'" value="B" >'+options[2]+'</label><label class="radio"> <input type="radio" name="'+single[i].id+'" value="C" >'+options[3]+'</label><label class="radio"> <input type="radio" name="'+single[i].id+'" value="D" >'+options[4]+'</label> <label class="radio"> 我的答案'+single[i].myanser+'：正确答案： '+single[i].answer+'</label></h5> </strong>';
        }
        $("#single_examlib").append(singlestr);
        for(var i=0;i<single.length;i++){
            var id=single[i].id;
    ...
    if(multiple!=null&&multiple!=""){
        ...
        for(var i=0;i<multiple.length;i++){
            var options = multiple[i].options.split("##")
            multiplestr+='<strong> <h4>'+multiple[i].sort+'、<span>'+multiple[i].title+'</span></h4><h5> <span> <label class="checkbox"> <input type="checkbox" name="'+multiple[i].id+'" value="A">'+options[1]+'</label><label class="checkbox"> <input type="checkbox" name="'+multiple[i].id+'" value="B">'+options[2]+'</label><label class="checkbox"> <input type="checkbox" name="'+multiple[i].id+'" value="C">'+options[3]+'</label><label class="checkbox"> <input type="checkbox" name="'+multiple[i].id+'" value="D">'+options[4]+'</label><label class="checkbox"> 我的答案'+multiple[i].myanser+'：正确答案:'+multiple[i].answer+'</label> </h5> </strong>';
        }
        $("#multiple_examlib").append(multiplestr);
        for(var i=0;i<multiple.length;i++){
    ...
    if(judge!=null&&judge!=""){
        ...
        $("#judge_type").html("判断题（共"+judge.length+"题，1题"+judge[0].weight+"分)");
        for(var i=0;i<judge.length;i++){
            judgestr+='<strong> <h4>'+judge[i].sort+'、<span>'+judge[i].title+'</span></h4><h5> <span> <label class="radio"> <input type="radio" name="'+judge[i].id+'" value="1">正确 </label><label class="radio"> <input type="radio" name="'+judge[i].id+'" value="0">错误 </label><label class="radio"> 我的答案'+judge[i].myanser+'：正确答案： '+(judge[i].answer==1?"正确":"错误")+'</label> </span> </h5> </strong>';
        }
        $("#judge_examlib").append(judgestr);
        for(var i=0;i<judge.length;i++){
            var id=judge[i].id;
            ...
            $("input[name=" +id + "]").each(function () {
                if ($(this).val() ==myanswer) {
                   ...
                   //填空
    if(tiangkong!=null&&tiangkong!=""){
        $("#tiangkong_total").val(judge.length);
        var judgestr=""
        $("#tiangkong_type").html("填空题（共"+judge.length+"题，1题"+tiangkong[0].weight+"分)");
        for(var i=0;i<tiangkong.length;i++){
        judgestr+='<strong> <h4>'+tiangkong[i].sort+'、<span>'+tiangkong[i].title+'</span></h4><h5> <input type="text" name="'+tiangkong[i].id+'" value=""> <label class="checkbox"> 我的答案'+tiangkong[i].myanser+' 正确答案:'+tiangkong[i].answer+'</label></h5> </strong>';
        }
        $("#tiangkong_examlib").append(judgestr);
        for(var i=0;i<tiangkong.length;i++){
            var id=judge[i].id;
            var myanswer=judge[i].myanswer;
            $("input[name=" +id + "]").each(function () {
