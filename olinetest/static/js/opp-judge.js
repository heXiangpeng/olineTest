...
$(function () {
    $("#btn_save").on("click", function () {
        ...
        $.ajax({
            type: "POST",
            url:"/examinee/examlibadd",
            data: "title="+title+"&options="+options+"&answer="+answer+"&Type="+type,
            dataType: "json",
...
examlib._examlib_datatable = function () {
    $("#examlib_datatable").dataTable({
        "sAjaxSource": global_RootPath+"/examinee/examlib/type/single?typle=judge",
        "sAjaxDataProp": "data",
        "aoColumns": [
            {"mDataProp": "id"},
            {"mDataProp": "title"},
...
examlib._btn_show_examlib=function(data) {
    $.ajax({
        ...
        success: function (datarc) {
            ...
            $("#show_answer").text(datarc.answer==1?"正确":"错误");
...
examlib._btn_edit_examlib = function (id) {
    $.ajax({
        ...
        dataType: "json",
        success: function (datarc) {
            $("#judgeId").val(datarc.id);
            $("#title").val(datarc.title);
            $("#answer").val(datarc.answer==1?"正确":"错误");
            $("#show_examlib_judge").hide();
            $("#add_examlib_judge").show();
            $("#list_examlib_judge").hide();
        }
