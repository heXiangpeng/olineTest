...
$(function () {
    ...
    $("#submit").on("click",function(){
        ...
        var judge_total=$("#judge_total").val();
         var tiangkong_total=$("#tiankong_total").val();
        ...
        if(judge_total!=""){
           ...
           if(tiangkong_total!=""){
            ...
            for(var j=0;j<tiangkong_total;j++){
                var record={};
                var rejr=$("#tiankong_examlib").find("h5").eq(j).find("input").val();
                if(rejr==""){
                    guodandan.failAlert("填空题-第"+(j+1)+"题未作答");
                    record={};
                    records=[];
                    return;
                }else{
                    var jr= $("#tiankong_examlib").find("h5").eq(j).find("input[type=text]");
                    var id= jr[0].name;
                    var val=jr[0].value;
                    alert(val);
                    record.id=id;
        ...
        var js = JSON.stringify(records);
        $.ajax({
            type: "POST",
            url: global_RootPath+"/uploadanser?eximid="+examid,
...
online._btn_start_exam = function (examid) {
    ...
    $.ajax({
        ...
        url: global_RootPath+"/records/init?examid=" + examid,
        dataType: "json",
        success: function (datarc) {
            if (datarc.rc == 0) {
                ...
                online.RenderExam(single,multiple,judge,tiangkong);
            } else {
...
online.RenderExam=function(single,multiple,judge,tiankong){
        $("#single_examlib").empty();
        $("#multiple_total").empty();
        $("#judge_total").empty();
         $("#tiankong_total").empty();
        //单选题
        if(single!=null&&single!=""){
            ...
            for(var i=0;i<single.length;i++){
                ...
                singlestr+='<div> <h4>'+single[i].sort+'、<span>'+single[i].title+'</span></h4><h5> <span> <label class="radio"> <input type="radio" name="'+single[i].id+'" value="A" >'+options[1]+'</label><label class="radio"> <input type="radio" name="'+single[i].id+'" value="B" >'+options[2]+'</label><label class="radio"> <input type="radio" name="'+single[i].id+'" value="C" >'+options[3]+'</label><label class="radio"> <input type="radio" name="'+single[i].id+'" value="D" >'+options[4]+'</label> </h5> </div>';
                var options = single[i].options.split("##")
            }
        ...
        if(multiple!=null&&multiple!=""){
            ...
            for(var i=0;i<multiple.length;i++){
                ...
                multiplestr+='<div> <h4>'+multiple[i].sort+'、<span>'+multiple[i].title+'</span></h4><h5> <span> <label class="checkbox"> <input type="checkbox" name="'+multiple[i].id+'" value="A">'+options[1]+'</label><label class="checkbox"> <input type="checkbox" name="'+multiple[i].id+'" value="B">'+options[2]+'</label><label class="checkbox"> <input type="checkbox" name="'+multiple[i].id+'" value="C">'+options[3]+'</label><label class="checkbox"> <input type="checkbox" name="'+multiple[i].id+'" value="D">'+options[4]+'</label> </h5> </div>';
            }
        ...
        if(judge!=null&&judge!=""){
           ...
           //填空
        if(tiankong!=null&&tiankong!=""){
            $("#tiankong_total").val(tiankong.length);
            var judgestr=""
            $("#tiankong_type").html("填空题（共"+tiankong.length+"题，1题"+tiankong[0].weight+"分)");
            for(var i=0;i<tiankong.length;i++){
                judgestr+='<strong> <h4>'+tiankong[i].sort+'、<span>'+tiankong[i].title+'</span></h4><h5> <input type="text" name="'+tiankong[i].id+'" value=""> </h5> </strong>';
            }
