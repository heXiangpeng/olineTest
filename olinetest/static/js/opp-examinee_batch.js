...
$(function () {
    ...
    $("#btn_save_group").on("click", function () {
        ...
        $.ajax({
            ...
            data: "groupname="+$.trim($("#groupname").val()),
            dataType: "json",
...
examinee_batch._btn_add_examinee=function(groupid,groupname){
    ...
    $("#groupid_next").val(groupid);
      if ($('#examinee_batch_datatable').hasClass('dataTable')) {
        dttable = $('#examinee_batch_datatable').dataTable();
        dttable.fnClearTable(); //清空一下table
        dttable.fnDestroy(); //还原初始化了的datatable
    }
    examinee_batch.examinee_batch_datatable(groupid);
    examinee_batch.show_list_examinee();
};
//删除分组
examinee_batch._btn_del_group = function (groupid) {
    layer.confirm('确定删除?', function () {
        $.ajax({
            type: "GET",
            url: global_RootPath+"/deletegroup?groupid=" + groupid,
            dataType: "json",
            success: function (data) {
...
examinee_batch.group_examinee_datatable = function (groupid) {
    $("#group_examinee_datatable").dataTable({
        "bDestory": true,
        "sAjaxSource": global_RootPath+"/examineegroup/get?groupid="+groupid+"&userid=" + new Date().getTime(),
        "sAjaxDataProp": "data",
        "aoColumns": [
            {"sDefaultContent": ''},
            {"mDataProp": "username"},
...
examinee_batch.btn_del_group_examinee=function(groupid,userid){
    layer.confirm('确定删除?', function () {
        $.ajax({
            type: "GET",
            url: global_RootPath+"/examineegroup/delete?userid="+ userid,
            dataType: "json",
            success: function (data) {
                guodandan.layerMsg(data, "删除成功", function () {
...
//获取正常状态的考生信息
examinee_batch.examinee_batch_datatable = function (groupid) {
    $("#examinee_batch_datatable").dataTable({
        "bDestory": false,
        "sAjaxSource": global_RootPath+"/AllByActStatus?groupid=" + groupid,
        "sAjaxDataProp": "data",
        "aoColumns": [
            {"sDefaultContent": ''},
            {"mDataProp": "username"},
