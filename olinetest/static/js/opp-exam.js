...
exam._topic_datatable = function () {
    $("#topic_datatable").dataTable({
        "sAjaxSource": "examinee/topic/getAll",
        "sAjaxDataProp": "data",
        "aoColumns": [
            {"sDefaultContent": ''},
            {"mDataProp": "name"},
...
exam._btn_edit_exam = function (id) {
    $.ajax({
        type: "GET",
        url: global_RootPath+"/exam/id?examid=" + id,
        dataType: "json",
        success: function (datarc) {
            $("#examid").val(datarc.id);
            $("#examname").val(datarc.examname);
            $("#groupid").val(datarc.groupid);
            $("#topicid").val(datarc.topicid);
            $("#groupname").val(datarc.groupname);
            $("#topicname").val(datarc.topicname);
            var newDate = new Date();
            newDate.setTime(datarc.begintime);
            $("#begintime_show").val(newDate.toLocaleDateString());
            $("#begintime").val(newDate.toLocaleDateString());
            newDate.setTime(datarc.endtime);
...
exam._btn_del_exam = function (id) {
    layer.confirm('确定删除?', function () {
        $.ajax({
            type: "GET",
            url: global_RootPath+"/exam/delete?examid=" + id,
            dataType: "json",
            success: function (datarc) {
                exam.clearTable_exam_datatable();
                layer.msg('删除成功！');
