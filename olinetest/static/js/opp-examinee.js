...
$(function () {
    ...
    $("#btn_save").on("click", function () {
        ...
        $.ajax({
            type: "POST",
            url: global_RootPath+"/examinee/add?userid="+$("#userid_e").val(),
            data: "username="+userName_j+"&ticketnumber="+ticketNumber_j+"&idcard="+idCard_j+"&password="+password_j+"&actstatus="+$('input[name="actstatus"]:checked').val()+"&phone="+$("#phone").val()+"&email="+$("#email").val()+"&address="+$("#address").val()+"&sex="+$('input[name="sex"]:checked').val(),
            dataType: "json",
            success: function (data) {
                if (data.staut==2){
                    guodandan.failAlert("用户名已经存在",function(){
...
$(function () {
    ...
    $("#btn_save_e").on("click", function () {
        ...
        var ticketNumber_j = $.trim($("#ticketnumber_e").val());
        var idCard_j = $.trim($("#idcard_e").val());
        ...
        $.ajax({
            type: "POST",
            url: global_RootPath+"/examinee/add?userid="+$("#userid_e").val(),
           data: "username="+userName_j+"&ticketnumber="+ticketNumber_j+"&idcard="+idCard_j+"&password="+password_j+"&actstatus="+$('input[name="actstatus"]:checked').val()+"&phone="+$("#phone_e").val()+"&email="+$("#email_e").val()+"&address="+$("#address_e").val()+"&sex="+$('input[name="sex_e"]:checked').val(),
...
examinee._examinee_datatable = function () {
    $("#examinee_datatable").dataTable({
        'bStateSave': true,
        "sAjaxSource": "/examinee/getAll?now=" + new Date().getTime(),
        "sAjaxDataProp": "data",
        "aoColumns": [
            {"mDataProp": "userid"},
            {"mDataProp": "username"},
...
examinee._btn_update_examineeactstatus = function (userid, actstatus) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "/examinee/actstatus/" + userid + "/" + actstatus,
        dataType: "json",
        success: function (data) {
...
examinee._btn_update_examinee = function (userid) {
    ...
    $.ajax({
        ...
        url:"/examinee/get?userid=" + userid,
        dataType: "json",
        success: function (data) {
            if (data.staut == 0) {
                $("#edit_examinee").show();
